...
function ShopVerwaltungContent({ user }) {
  ...
  const [gruppen, setGruppen] = useState([]); // Haesgruppen
  ...
  const [formSparten, setFormSparten] = useState([]); // haesgruppe_ids
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const [ordersRes, articlesRes, membersRes, gruppenRes] = await Promise.all([
  ...
  const getGruppenName = (id) => {
    const g = gruppen.find((g) => g.id === id);
    return g ? g.name || g.bezeichnung : '';
  };
  const formatEuro = (n) => {
  ...
  const addCustomGroesse = () => {
    ...
    if (val && !formGroessen.includes(val)) setFormGroessen((prev) => [...prev, val]);
  ...
  const handleSaveArtikel = async (e) => {
    ...
    const data = {
      ...
      preis: parseFloat(formPreis) || 0, groessen: formGroessen,
      artikel_nummer: formArtikelNummer,
      sparten: formSparten.length > 0 ? formSparten : null,
      aktiv: formAktiv, sortierung: parseInt(formSortierung, 10) || 0
    };
    try {
      if (editingArtikel) {
  ...
  const filteredBestellungen = useMemo(() => {
    return bestellungen.filter((o) => {
      ...
      return name.includes(search.toLowerCase()) && (statusFilter === 'Alle' || o.status === statusFilter);
    });
  ...
  // SAMMELBESTELLUNG — gruppiert nach Artikel + Sparte/Design + Größe
  const sammelbestellungData = useMemo(() => {
    ...
    bestellungen.forEach((o) => {
      ...
      const items = Array.isArray(o.positionen) ? o.positionen : [];
      items.forEach((item) => {
        // Key inklusive Sparte/Design!
        const sparte = item.sparte || 'Ohne Sparte';
        const key = `${item.artikel_name || ''}_${sparte}_${item.groesse || ''}`;
        if (!agg[key]) {
          agg[key] = {
    ...
    return Object.values(agg).sort((a, b) => {
      ...
      if (c !== 0) return c;
      const sc = a.sparte.localeCompare(b.sparte);
      if (sc !== 0) return sc;
  ...
  const handleExportBestellungen = () => {
    ...
    const rows = filteredBestellungen.map((o) => {
      ...
      const itemsText = Array.isArray(o.positionen) ? o.positionen.map((p) => `${p.artikel_name} [${p.sparte || 'Ohne'}] (${p.groesse}): ${p.menge}x`).join(', ') : '';
      return [o.id || '', mName, date, o.gesamtbetrag || 0, o.zahlungsart || '', o.status || '', itemsText];
    });
    exportCSV(headers, rows, 'Shop_Bestellungen.csv');
  };

  // Export für Dienstleister — mit Sparte/Design-Spalte!
  const handleExportSammelbestellung = () => {
    const headers = ['Artikel', 'Sparte/Design', 'Größe', 'Gesamtmenge', 'Einzelpreis', 'Gesamtsumme'];
    const rows = sammelbestellungData.map((s) => [s.artikel_name, s.sparte, s.groesse, s.menge, s.einzelpreis, s.gesamt]);
    exportCSV(headers, rows, 'Sammelbestellung_Lieferant.csv');
  };

  const handleExportPacklisten = () => {
    const headers = ['Mitglied', 'Zahlungsstatus', 'Gesamtartikel', 'Gesamtsumme', 'Details (Empfänger - Artikel - Sparte/Design - Größe - Menge)'];
    const rows = packlistenData.map((g) => {
      ...
      const details = g.orders.flatMap((o) => Array.isArray(o.positionen) ? o.positionen : [])
        .map((p) => `${p.fremdname || 'Eigenbedarf'} (${p.artikel_name} - ${p.sparte || 'Ohne'} - ${p.groesse} - ${p.menge}x)`).join(' | ');
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white p-4 sm:p-6 md:p-8">
      ...
      {loading && (<div className="flex flex-col items-center justify-center py-20"><div className="w-10 h-10 border-[3px] border-border border-t-primary rounded-full animate-spin"></div><p className="text-gray-400 mt-4">Daten werden geladen...</p></div>)}
      {error && !loading && (<div className="bg-primary/10 border border-primary/30 rounded-xl p-4 flex items-center gap-3 mb-6"><AlertCircle className="w-5 h-5 text-primary shrink-0" /><p className="text-white">{error}</p></div>)}
                          ...
                          <div onClick={() => setExpandedOrders((prev) => ({ ...prev, [o.id]: !prev[o.id] }))} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-400 mt-1"><span>{dateStr}</span><span>•</span><span>{itemsCount} Artikel</span><span>•</span><span>Zahlung: {o.zahlungsart || 'N/A'}</span></div>
                              </div>
                            </div>
                                  ...
                                  {(o.positionen || []).map((item, idx) => (
                                    <div key={idx} className="flex items-center justify-between text-sm py-1.5 border-b border-border last:border-0">
                                      <div>
                                        ...
                                        <p className="text-xs text-gray-400">Größe: {item.groesse} | Für: {item.fremdname || 'Eigenbedarf'}{item.sparte ? ` | Sparte/Design: ${item.sparte}` : ''}</p>
                                      </div>
          ...
          {tab === 'artikel' && (
            <div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {artikel.length === 0 ? <div className="col-span-full p-8 text-center bg-card border border-border rounded-xl text-gray-500">Keine Artikel vorhanden. Erstellen Sie den ersten Artikel oben rechts.</div> : (
                  artikel.map((art) => (
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-lg my-8 relative shadow-2xl">
            ...
            <form onSubmit={handleSaveArtikel} className="space-y-4">
              ...
              {gruppen.length > 0 && (
                <div>
                  <label className="block text-xs font-semibold uppercase text-gray-400 mb-2">Sparten/Designs (optional — leer = für alle sichtbar)</label>
